...
$string['optioncardgroup_help'] = 'When importing as editable content, render each run of two or more ordinary pictures as a Bootstrap card group — the same markup the tiny_bootstrap editor plugin inserts — instead of a plain image grid. Each picture becomes a card that opens a click-to-enlarge zoom, and a paired short caption becomes the card text. A slide with a single picture is always shown as a centred, height-capped figure rather than a one-card group. Reconstructed diagrams (SmartArt and shape flows) are unaffected. This option applies to the editable import only; it has no effect when importing as faithful images.';
...
$string['optionsmartartimages'] = 'Keep SmartArt slides as images';
$string['optionsmartartimages_help'] = 'SmartArt diagrams cannot be turned into editable text without losing their meaning, so with this on any slide containing SmartArt is kept as a faithful rendered image instead of a flattened bullet list. Every other slide is still imported as editable content. This needs the same LibreOffice render backend as the "faithful images" mode, and applies to the editable import only.';
