...
$string['cloudenabled'] = 'Use the external render service';
$string['cloudenabled_desc'] = 'When enabled and a service URL and key are set, faithful-image and complex-slide rendering are produced by the external render service instead of a local LibreOffice install. Slide content is sent to that service for rendering.';
$string['cloudheading'] = 'External render service';
$string['cloudheading_desc'] = 'Optional. Render faithful slide images with an external service, so image import works without LibreOffice and poppler on this server. The importer reconstructs each slide as HTML and posts it to the service, which returns a page image.';
$string['cloudkey'] = 'API key';
$string['cloudkey_desc'] = 'The bearer key the render service requires.';
$string['cloudurl'] = 'Service URL';
$string['cloudurl_desc'] = 'Base URL of the render service (for example https://render.example.com).';
...
$string['errorcloudrender'] = 'The external render service did not return the slide images. Please check the service URL, key, and that the service is running.';
...
$string['optionrenderfont'] = 'Render font';
$string['optionrenderfont_help'] = 'When a slide is turned into an image — the "faithful images" mode, or a kept SmartArt/complex slide — force this font on the deck before it is rendered. Use it when the deck\'s own font is not installed on the server and text overflows its boxes in the rendered image: a deck in Aptos (Office\'s current default) or Calibri renders cleanly with Carlito, for example. "Keep original" leaves the deck\'s fonts untouched. The chosen font must be installed where LibreOffice runs, and this affects only the rendered images — never the editable text.';
...
$string['privacy:metadata'] = 'The PowerPoint import tool does not store any personal data of its own. It creates book chapters and files, which are stored and managed by the Book activity. An uploaded presentation is staged temporarily while an import is confirmed or queued, and staged copies are deleted after the import (or automatically cleaned up when abandoned).';
$string['privacy:metadata:pptx_render_service'] = 'When the optional external render service is configured, reconstructed slide content is sent to it to be rendered into page images. No data is stored by the service beyond the lifetime of the request.';
$string['privacy:metadata:pptx_render_service:slidecontent'] = 'The reconstructed slide content (text and images) sent to the render service to produce a page image.';
$string['renderfontkeep'] = 'Keep original';
