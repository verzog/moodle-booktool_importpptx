...
class import_form extends \moodleform {
    ...
    protected function definition() {
        ...
        if (!empty($this->_customdata['officeenabled'])) {
            $fonts = ['' => get_string('renderfontkeep', 'booktool_importpptx')];
            foreach (\booktool_importpptx\office\renderer::RENDER_FONTS as $family) {
                $fonts[$family] = $family;
            }
            $mform->addElement('select', 'renderfont', get_string('optionrenderfont', 'booktool_importpptx'), $fonts);
            $mform->setDefault('renderfont', '');
            $mform->addHelpButton('renderfont', 'optionrenderfont', 'booktool_importpptx');
