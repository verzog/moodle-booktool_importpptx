...
namespace booktool_importpptx\pdf;
...
class renderer {
    ...
    public function render_pages(\stored_file $pdf, int $maxdim): \Generator {
        ...
        if ($count > self::MAX_PAGES) {
            throw new \moodle_exception('errortoomanypages', 'booktool_importpptx', '', (object) [
                'count' => $count,
                'max' => self::MAX_PAGES,
            ]);
        }
    ...
    public function render_path(string $path, int $maxdim): \Generator {
        ...
        if ($result['code'] !== 0) {
            throw new \moodle_exception('errorpdfrender', 'booktool_importpptx');
        }
        ...
        foreach ($pages as $file) {
            ...
            [$ext, $web] = \booktool_importpptx\image_helper::to_web($bytes, $maxdim);
    ...
    private static function binary(string $name): string {
        $dir = trim((string) get_config('booktool_importpptx', 'popplerpath'));
