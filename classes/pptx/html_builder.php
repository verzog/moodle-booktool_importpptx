...
class html_builder {
    ...
    private function build_section(\stdClass $parsed): \stdClass {
        ...
        if (!empty($lines)) {
            $plate = '<div class="col-12 col-md-3"><div class="booktool-importpptx-plate" style="background-color:'
                . $colour . ';">' . implode('<br>', $lines) . '</div></div>';
            $hero = '<div class="container-fluid booktool-importpptx-section"><div class="row">'
                . $plate . '<div class="col-12 col-md-9 booktool-importpptx-lede">' . $content . '</div></div></div>';
        } else {
            $hero = '<div class="booktool-importpptx-lede">' . $content . '</div>';
        }
        $html = trim($hero);
    ...
    private function render_columns(array $band): string {
        ...
        return self::row_wrap('<div class="row g-3 mb-3 booktool-importpptx-cols">' . $cells . '</div>');
    }
    ...
    private function render_cell(block $b, int $textsize = 0): string {
        if ($b->type === block::TYPE_IMAGE) {
            ...
            if ($this->cardgroup) {
                ...
                return '<div class="booktool-importpptx-card">' . $card . '</div>' . $modal;
            }
    ...
    private function render_audio(string $mediapath): string {
        ...
        $fallback = s(get_string('audiounsupported', 'booktool_importpptx'));
        return '<audio class="booktool-importpptx-audio" controls preload="none">'
    ...
    private function render_figure(string $ref, int $widthpct = 0): string {
        ...
        return '<div class="booktool-importpptx-figure"><img src="' . $ref . '" alt="" class="img-fluid"' . $style . '></div>';
    }
    ...
    private function render_grid(array $imgs, ?array $caps): string {
        ...
        return self::row_wrap('<div class="row g-3 booktool-importpptx-grid">' . $cells . '</div>');
    }
    ...
    private static function row_wrap(string $rowhtml): string {
        return '<div class="container-fluid booktool-importpptx-rowwrap">' . $rowhtml . '</div>';
    }
    ...
    private function build_image_card(string $ref, string $caption = ''): array {
        ...
        // several chapters (each built separately) render on one page, as in the
        // Print book view — a per-chapter counter would repeat and cross-wire.
        $uid = \html_writer::random_id('bookImportCard');
        $enlarge = s(get_string('clicktoenlarge', 'booktool_importpptx'));
        $body = $caption === ''
            ? ''
    ...
    private function render_card_group(array $imgs, ?array $caps): string {
        ...
        foreach ($imgs as $idx => $ref) {
            ...
            $cards[] = '<div class="col booktool-importpptx-card">' . $card . '</div>';
            $modals[] = $modal;
        }
        $row = self::row_wrap('<div class="row ' . $rowcols . ' g-4 booktool-importpptx-cardgroup">'
            . implode('', $cards) . '</div>');
        return $row . "\n" . implode("\n", $modals);
    }
     ...
     * chapter view page (where the editor plugin's own CSS is not loaded); the
     * theme's bundled Bootstrap JavaScript drives the open/close behaviour.
     *
    ...
    private function render_card_modal(string $uid, string $ref, string $caption): string {
        ...
        return '<div class="modal fade tiny-bootstrap-modal booktool-importpptx-cardmodal" id="'
